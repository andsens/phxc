...
main() {
  ...
  DOC="boot-disk.sh - Boot images
Usage:
  boot-disk.sh HOSTNAME
"
# docopt parser below, refresh this parser with `docopt.sh boot-disk.sh`
# shellcheck disable=2016,1090,1091,2034
docopt() { source "$PKGROOT/.upkg/andsens/docopt.sh/docopt-lib.sh" '1.0.0' || {
ret=$?; printf -- "exit %d\n" "$ret"; exit "$ret"; }; set -e
trimmed_doc=${DOC:0:57}; usage=${DOC:27:30}; digest=3eb13; shorts=(); longs=()
argcounts=(); node_0(){ value HOSTNAME a; }; node_1(){ required 0; }; node_2(){
required 1; }; cat <<<' docopt_exit() { [[ -n $1 ]] && printf "%s\n" "$1" >&2
printf "%s\n" "${DOC:27:30}" >&2; exit 1; }'; unset var_HOSTNAME; parse 2 "$@"
...
# docopt parser above, complete command for generating this parser is `docopt.sh --library='"$PKGROOT/.upkg/andsens/docopt.sh/docopt-lib.sh"' boot-disk.sh`
  eval "$(docopt "$@")"
