...
apt-get -y update
apt-get -y install --no-install-recommends jq curl ca-certificates
...
trap "rm \"$temp\"" EXIT
curl -Lso"$temp" "https://github.com/orbit-online/upkg/releases/download/v0.28.2/upkg-install.tar.gz"
