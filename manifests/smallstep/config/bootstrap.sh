...
setup_issuer_provisioner() {
  ...
  if $jwk_is_new || ! diff -q "$STEPPATH/persistent-certs/root_ca.crt" "$STEPPATH/certs/step-issuer-provisioner/caBundle.key"; then
...
spec:
  ...
  provisioner:
    ...
    passwordRef:
      ...
      key: password" "$(base64 -w0 "$STEPPATH/persistent-certs/root_ca.crt")" "$(step crypto jwk thumbprint < "$STEPPATH/certs/step-issuer-provisioner/pub.json")"
    )
  else
...
setup_ssh_host_provisioner() {
  ...
  if ! kubectl get -n "$NAMESPACE" secret ssh-host-provisioner -o jsonpath='{.data.pub\.json}' | base64 -d >"$STEPPATH/certs/ssh-host-provisioner/pub.json"; then
