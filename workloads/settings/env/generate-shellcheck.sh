...
main() {
  ...
  DOC="generate-shellcheck.sh - Generate a settings file for shellcheck
Usage:
  generate-shellcheck.sh
"
# docopt parser below, refresh this parser with `docopt.sh generate-settings-env.shellcheck.sh`
# shellcheck disable=2016,2086,2317,1090,1091,2034
docopt() { local v='2.0.1'; source \
"$PKGROOT/.upkg/docopt-lib-v$v/docopt-lib.sh" "$v" || { ret=$?;printf -- "exit \
%d\n" "$ret";exit "$ret";};set -e;trimmed_doc=${DOC:0:96};usage=${DOC:65:31}
digest=400d4;options=();node_0(){ return 0;};cat <<<' docopt_exit() { [[ -n $1 \
]] && printf "%s\n" "$1" >&2;printf "%s\n" "${DOC:65:31}" >&2;exit 1;}';local \
