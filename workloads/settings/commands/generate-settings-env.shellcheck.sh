...
ret=$?;printf -- "exit %d\n" "$ret";exit "$ret";};set -e
trimmed_doc=${DOC:0:122};usage=${DOC:78:44};digest=53efe;options=();node_0(){
return 0;};cat <<<' docopt_exit() { [[ -n $1 ]] && printf "%s\n" "$1" >&2
printf "%s\n" "${DOC:78:44}" >&2;exit 1;}';local varnames=() varname;for \
...
# docopt parser above, complete command for generating this parser is `docopt.sh --library='"$PKGROOT/.upkg/docopt-lib.sh/docopt-lib.sh"' generate-settings-env.shellcheck.sh`
  eval "$(docopt "$@")"
